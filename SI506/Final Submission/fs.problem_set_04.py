...
def num_chars_in_file(aFileName):
    lengthth=0
    with open(aFileName) as source:
        lines=source.readlines()
        for eachLine in lines:
            lengthth+=len(eachLine)
    return lengthth
...
def num_lines_in_file(aFileName):
    with open(aFileName) as source:
        lines=source.readlines()
    return len(lines)
...
def create_nested_list(aFileName='planets.txt'):
    noHeaderData=[]
    with open(aFileName,'r') as source:
        lstOfData = source.readlines()[1:]
        for eachLine in lstOfData:
            words = eachLine.split(',')
            noHeaderData.append(words)
    return noHeaderData
...
def middle_planets(aNestedList=planet_data):
    planet200to1500=[]
    for eachPlanet in aNestedList:
        if float(eachPlanet[4])>=200 and float(eachPlanet[4])<=1500:
                planet200to1500.append(eachPlanet[0])
    return planet200to1500
...
def string_from_list(aList):
    strWithComa=','.join(aList)
    # for item in aList:
    #     strwithcoma=','.join()
    newCharacter=strWithComa+'\n'
    return newCharacter
...
# with fout as target:
#     for eachLine in lines:
#         target.write(string_from_list(eachLine))
