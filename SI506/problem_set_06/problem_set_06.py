...
Each function will take a list of strings as the first and only argument, and return a transformed list of strings as the returned value
...
def transform_a_list_even(list_of_strings):
    ...
    #     if lstEven.index(item)%2 == 0:
    #         lstEven.append(item)
    for index in range(len(list_of_strings)):
        if index%2 != 0:
            lstEven.append(list_of_strings[index])
    for eachStr in lstEven:
        eachStr.replace('- ','')
    return lstEven
...
def transform_a_list_odd(list_of_strings):
    ...
    lstOfPunctuation = ["-", "...", ",",".", "?", "!"]
    for eachStr in lstOdd:
        for eachPunctuation in lstOfPunctuation:
            eachStr = eachStr.replace(eachPunctuation,'')
    return lstOdd
...
for i in range(len(transformed_file_lines_list_odd)):
    newlst.append(transformed_file_lines_list_odd.replace('\n','')[i]+"|"+transformed_file_lines_list_even[i])
...
def output_words_frequency_from_file(source_file_path, target_file_path, topic_words_list):
    ...
    with open (source_file_path,"r") as source:
        for eachLine in source.readlines():
